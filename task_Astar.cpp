...
void path_planning(Mat img,int **h)
{
  int c=0;
  node **ar;
	  ...
	  for(int j=n.y-1;j<=n.y+1;j++)
		...
		continue;
	      if(mark.at<uchar>(i,j)==0)
	     {
	       if(i==gx && j==gy)
		 {
		   ar[i][j].cost=ar[n.x][n.y].cost+10+h[i][j];
		   ar[i][j].px=n.x;
		   ar[i][j].py=n.y;
		   goto jmp;
		 }
	       ar[i][j].cost=(i==n.x || j==n.y)?ar[n.x][n.y].cost+10+h[i][j]:ar[n.x][n.y].cost+10+h[i][j];
	      ar[i][j].px=n.x;
	      ar[i][j].py=n.y;
	      mark.at<uchar>(i,j)=1;
	      s.push(ar[i][j]);
